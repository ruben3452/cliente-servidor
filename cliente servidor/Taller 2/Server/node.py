...
def OperacionesToquens(message, ml, hostsucc, portsucc, limsucc, hostpre, portpre, LimPred, s2):

    if message["op"] == "adicionar":

        if LimToq(ml, LimPred, message["toquen"]):
        ...
        if LimToq(ml, LimPred, message["indice"]):
...
if myhost==hostser:
    ...
    succSocket1.send_json({'op': 'new', 'key': nodeName, 'key1': myhost, 'key2': porhost})
    succSocket1.close()
...
while True:
    ...
    if mySocket in socks and socks[mySocket] == zmq.POLLIN:
        ...
        if m['op'] == 'new':
            ...
            OperacionesToquens(m, MyLim, hostsucc, portsucc, limsucc, hostpre, portpre, LimPred,s2)
            s2.close()
